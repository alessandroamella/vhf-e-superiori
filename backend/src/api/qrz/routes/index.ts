  ...
  async (req, res) => {
    try {
      ...
      if (req.query.geocode && info.lat && info.lon) {
        ...
        const geocode = await location.reverseGeocode(info.lat, info.lon);
        if (geocode) {
      ...
      if (err instanceof Error && err.message === Errors.QRZ_OM_NOT_FOUND) {
      ...
      logger.error(err);
      res.status(INTERNAL_SERVER_ERROR).json(createError(Errors.UNKNOWN_ERROR));
    }
  },
);
