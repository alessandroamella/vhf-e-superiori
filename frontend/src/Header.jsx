...
const Header = () => {
  ...
  const impersonatedUserId = useUserStore((store) => store.impersonatedUserId);
  const stopImpersonation = useUserStore((store) => store.stopImpersonation);
  const location = useLocation();
  ...
  const handleStopImpersonation = () => {
    stopImpersonation();
    window.location.href = "/eventmanager"; // Torna all'admin
  };

  return (
    <>
      {impersonatedUserId && (
        <div className="w-full bg-red-600 text-white text-center py-2 px-4 flex justify-between items-center font-bold animate-pulse">
          <div className="flex items-center gap-2 mx-auto">
            <FaUserSecret size={20} />
            <span>
              MODALITÀ IMPERSONATION ATTIVA: Stai agendo come {user?.callsign}
            </span>
          </div>
          <Button
            size="xs"
